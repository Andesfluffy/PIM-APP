    ...
    <main className="relative min-h-screen overflow-hidden bg-gradient-to-br from-tea-green-900 via-naples-yellow-900 to-red-crayola-900 text-oxford-blue-500">
      ...
      <AnimatePresence mode="wait">
        {!isAuthenticated && view === "auth" ? (
          ...
          >
            <div className="relative flex w-full max-w-5xl flex-col justify-center gap-12 rounded-[36px] border border-tea-green-300/50 bg-white/85 px-6 py-10 shadow-[0_60px_120px_-70px_rgba(1,25,54,0.45)] backdrop-blur-xl sm:px-10 lg:flex-row lg:items-center lg:gap-14 lg:px-16">
              <div className="flex w-full flex-col gap-10 lg:w-[48%]">
                <div className="space-y-6">
                  <div className="inline-flex items-center gap-3 rounded-full bg-naples-yellow-900 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-oxford-blue-400 sm:text-sm">
                ...
                <div className="space-y-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    className="type-subtle inline-flex items-center justify-center rounded-2xl border border-charcoal-900 px-4 py-3 text-sm font-semibold text-oxford-blue-400 transition-all duration-200 hover:border-oxford-blue-500 hover:text-oxford-blue-500"
                  >
                    Tour the live suite
                  </button>
                </div>
              ...
              <div className="grid grid-cols-1 gap-4 text-sm text-charcoal-500/80 sm:grid-cols-2 lg:grid-cols-3">
                {quickMetrics.map((metric) => (
                  <div
                    key={metric.label}
                    className="rounded-2xl border border-tea-green-800 bg-tea-green-900 p-4 text-center"
                  >
                    <p className="text-xs uppercase tracking-[0.2em] text-charcoal-500">{metric.label}</p>
                    <p className="mt-2 text-lg font-semibold text-oxford-blue-400">{metric.value}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid w-full flex-1 gap-4 lg:grid-rows-[minmax(0,1fr)_minmax(0,1fr)]">
              <div className="relative h-56 overflow-hidden rounded-[28px] border border-charcoal-900 sm:h-64">
                  <Image
                    src={heroImage}
                ...
                <div className="absolute bottom-6 left-6 right-6 rounded-2xl bg-white/85 p-4 text-oxford-blue-500 shadow-[0_25px_45px_-35px_rgba(1,25,54,0.55)] backdrop-blur">
                  <p className="text-[11px] uppercase tracking-[0.4em] text-charcoal-500">Preview</p>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {featureTiles.map((feature) => (
                    <div
                      key={feature.key}
                      className="group relative flex min-h-[180px] flex-col justify-between overflow-hidden rounded-3xl border border-tea-green-700 bg-white/80 p-6 shadow-[0_32px_60px_-45px_rgba(1,25,54,0.45)] backdrop-blur-md"
                    >
                      <div
                      ...
                      <div className="relative z-10 space-y-4">
                        <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-white/70 text-2xl shadow-sm shadow-charcoal-900/70">
                          {feature.icon}
                        </span>
