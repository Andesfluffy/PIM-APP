...
app.http('getNotes', {
    ...
    handler: async (request, context) => {
        try {
            ...
            const notes = await database.findByUserId('notes', userId);
    ...
    handler: async (request, context) => {
        try {
            ...
            if (!noteData.text || !noteData.text.trim()) {
                return {
                    status: 400,
                    body: JSON.stringify({ error: 'Note text is required' })
                };
            }

            const note = await database.create('notes', {
                ...noteData,
                userId,
...
app.http('updateNote', {
    ...
    handler: async (request, context) => {
        try {
            ...
            const success = await database.updateById('notes', noteId, userId, {
...
app.http('deleteNote', {
    ...
    handler: async (request, context) => {
        try {
            ...
            const success = await database.deleteById('notes', noteId, userId);
...
// File: functions/contacts-api/index.js
const { app } = require('@azure/functions');
const database = require('../shared/database');
const auth = require('../shared/auth');
